...
async def logging_middleware(request: Request, call_next):
    ...
    logger.info(
        ...
        extra={"correlation_id": correlation_id, "method": request.method, "url": str(request.url)}
    )
...
async def startup_event():
    ...
    try:
        ...
        if settings.is_production:
            logger.error(f"Failed to initialize rate limiting in production: {e}")
            raise
        else:
            logger.warning(f"Rate limiting initialization failed in development: {e}")
...
# Create APIException alias for routers (maintaining backward compatibility)
...
async def startup_event():
    ...
    # Run security validation FIRST - fail fast on security issues
    try:
        ...
        logger.warning("Security validator not found - skipping validation")
    except SystemExit:
