...
class QuestDBDataLoader:
    ...
    def load_ohlcv(self, symbol: str, start_date: datetime, end_date: datetime,
        ...
        # Market data already has OHLCV columns
        query = f"SELECT timestamp, open, high, low, close, volume FROM market_data WHERE symbol = '{symbol}' AND timestamp >= '{start_str}' AND timestamp < '{end_str}' ORDER BY timestamp"
    ...
    def get_available_symbols(self, min_bars: int = 100) -> List[str]:
        ...
        query = f"SELECT symbol, COUNT(*) as bar_count FROM market_data GROUP BY symbol HAVING COUNT(*) >= {min_bars} ORDER BY bar_count DESC"
        df = self._execute_query(query)
        if df is None:
            return []
    ...
    def get_date_range(self, symbol: str) -> Optional[tuple]:
        ...
        query = f"SELECT MIN(timestamp) as min_date, MAX(timestamp) as max_date FROM market_data WHERE symbol = '{symbol}'"
        df = self._execute_query(query)
        if df is None or len(df) == 0:
            return None
